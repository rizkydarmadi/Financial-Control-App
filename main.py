...
def show(s:str=None):
    try:
        ...
        print(colored(f"""| {'***'.ljust(5)} | {'TOTAL'.ljust(20)} | {str(total_debit).ljust(20)} | {str(total_credit).ljust(20)}  | {str(total_saldo).ljust(20)} | {'IS BALANCE'.ljust(50)} {'|'.ljust(0)}""",'cyan','on_grey')if (total_credit-total_debit) == total_saldo else colored(f"""| {'***'.ljust(5)} | {'TOTAL'.ljust(20)} | {str(total_debit).ljust(20)} | {str(total_credit).ljust(20)}  | {str(total_saldo).ljust(20)} | {'NOT BALANCE (PLEASE CHECK YOUR TRANSACTION)'.ljust(50)} {'|'.ljust(0)}""",'white','on_red') )
