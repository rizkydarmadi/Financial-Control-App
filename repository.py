...
class Repository:
    @staticmethod
    def get(
        limit: int = None, stardate: str = None, enddate: str = None, terms: str = None
    ):
    ...
    def update(id: int, amount: int = None, deskripsi: str = None):
        with Session() as session:
            ...
            before_stmt = (
                Select(Finance).where(Finance.id < id).order_by(Finance.id.desc())
            )
    ...
    def total_debit():
        with Session() as session:
            stmt = Select(func.sum(Finance.debit))
            data = session.execute(stmt).scalar()
    ...
    def total_credit():
        with Session() as session:
            stmt = Select(func.sum(Finance.credit))
            data = session.execute(stmt).scalar()
